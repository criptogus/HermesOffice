...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'hermesoffice-slides-temp-test-'))
  roots.push(root)
  return root
}
...
describe('generated-page temp sweep', () => {
  it('removes only expired .pptx files matching the UUID pattern', async () => {
    ...
    const cloudDir = join(root, 'hermesoffice-cloud-pages')
    const localDir = join(root, 'hermesoffice-local-pages')
  ...
  it('skips directories and non-.pptx files', async () => {
    const root = await tempRoot()
    await mkdir(join(root, 'hermesoffice-cloud-pages'), { recursive: true })

    const dirLike = join(root, 'hermesoffice-cloud-pages', 'not-a-file')
