...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/HermesOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'HermesOffice'))
    expect(existsSync(join(documents, 'HermesOffice'))).toBe(true)
  })
})
